...
test("initGoogleAutoLogin applies cached entitlement and skips prompt when token already exists", async () => {
  await withMockedLocalStorage(async (data) => {
    ...
    uiEntitlementSignInMethods.initGoogleAutoLogin.call(context as never);
...
test("initGoogleAutoLogin starts retry flow and credential callback persists token", async () => {
  await withMockedLocalStorage(async (data) => {
...
test("promptGoogleSignIn reuses existing token without opening Google prompt", async () => {
  await withMockedLocalStorage(async (data) => {
    stubWindow();
    data.set("whatfees_google_id_token", "already-signed-in");
    const context = createContext();
...
test("promptGoogleSignIn tolerates suppressed identity prompt without error notification", async () => {
  await withMockedLocalStorage(async () => {
    requestGoogleIdentityPromptMock.mockReturnValue(false);
    const initialize = vi.fn((config: { callback: (response: { credential?: string }) => void }) => {
      // Keep callback wired to ensure initialize path executed.
...
test("promptGoogleSignIn warns when Google identity API is unavailable", async () => {
  await withMockedLocalStorage(async () => {
    stubWindow();
    const context = createContext();
    ...
    assert.equal((context.notify as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0], "Google sign-in is not available yet. Please try again.");
    assert.equal((context.notify as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[1], "warning");
  });
});
...
test("promptGoogleSignIn ignores empty credential callback and handles initialize exception", async () => {
  await withMockedLocalStorage(async (data) => {
    ...
    const throwingInitialize = vi.fn(() => {
      throw new Error("init failed");
    });
    stubWindow({ initialize: throwingInitialize, prompt: vi.fn() });
    uiEntitlementSignInMethods.promptGoogleSignIn.call(context as never);
    assert.equal((context.notify as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0], "Google sign-in failed to open. Please try again.");
  });
});

test("openVerifyPurchaseModal respects manual mode and token requirements", async () => {
  await withMockedLocalStorage(async (data) => {
    const manualOff = createContext({
      showManualPurchaseVerify: false,
      showVerifyPurchaseModal: false
    });
    uiEntitlementSignInMethods.openVerifyPurchaseModal.call(manualOff as never);
    ...
    uiEntitlementSignInMethods.openVerifyPurchaseModal.call(missingToken as never);
    ...
    uiEntitlementSignInMethods.openVerifyPurchaseModal.call(withToken as never);
